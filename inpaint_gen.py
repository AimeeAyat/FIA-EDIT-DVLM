...
def _resolve_transformer_config(weights_dir):
    # Prefer weights/transformer/ (directory with config.json), fall back to standalone file
    import shutil
...
def load_models(args, dtype=torch.bfloat16):
    ...
    # RTX 5090 (32 GB) and similar sub-40 GB GPUs can't hold FLUX transformer (~23.8 GB)
    # + T5 (~9.4 GB) simultaneously; cpu_offload keeps transformer on GPU for all 28 steps
    # and only moves T5/VAE over PCIe (~2 s overhead total).
...
def generate_image(pipe, img_config, param_config, output_dir):
    ...
    for param_idx,param in enumerate(param_config['params']):
        ...
        edit_idx=edit_mask_parser(mask_image,height=height,width=width,cascade_num=param['cascade_num'])
