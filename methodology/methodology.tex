...
Grounding DINO~\citep{liu2023grounding} localises the object region via text-conditioned
bounding-box prediction; SAM~2~\citep{ravi2024sam2} then produces a precise binary object
mask $\mathbf{M}_B$.
...
detail---we pass it directly through FLUX's VAE encoder $\mathcal{E}$,
...
pass~\citep{Zhu2025KVEdit}, we preserve the background through a single clean forward pass,
...
is inserted---a property that hard KV-freezing~\citep{Zhu2025KVEdit} suppresses but that
photorealistic compositing requires.
Unlike FIA-Edit~\citep{FIAEdit2024}, which applies the same frequency fusion over the entire
...
numerical error that corrupts even unedited regions.
...
Unlike FSI-Edit~\citep{FSIEdit2025}, which injects noise globally and risks content bleeding
