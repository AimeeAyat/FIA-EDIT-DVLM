...
class DoubleStreamBlock_rf(DoubleStreamBlock):
    def __init__(self, hidden_size: int, num_heads: int, mlp_ratio: float, qkv_bias: bool = False):
...
class SingleStreamBlock_rf(SingleStreamBlock):
    def __init__(
        self,
        hidden_size: int,
        num_heads: int,
    ...
    def forward(self, x: Tensor, vec: Tensor, pe: Tensor, info) -> tuple[Tensor, dict]:
        ...
        if info['inject'] and info['id'] > 19:
            ...
            if info['inverse']:
                ...
                v = info['feature'][feature_name].to(x.device)
...
class DoubleStreamBlock_kv(DoubleStreamBlock):
    ...
    def forward(self, img: Tensor, txt: Tensor, vec: Tensor, pe: Tensor, info) -> tuple[Tensor, Tensor]:
        ...
        if info['inverse']:
            ...
            del info['feature'][feature_k_name]
            del info['feature'][feature_v_name]
