...
The working commit is 8805824.

To revert to it later:


git checkout 8805824 -- dvlm/ EEdit_colab_dvlm.ipynb
Or to see exactly what's in it:


git show 8805824 --stat
...
# No FLUX, no EEdit, no dataset — only SAM + Gradio
!python gradio_sam_smoke.py --share


%cd /content/EEdit
%pip install -q gradio

# --share creates a public tunnel — required on Colab (no open ports)
# SAM checkpoint (~2.4 GB) downloads automatically on first run
!python gradio_app.py --weights ./weights --share
...
Dropdown — "Choose style…" is now the first option in both apps. Its value maps to RC internally so prompts show immediately on load without the user needing to select anything.

Prompt boxes pre-populated — Both the augmented positive prompt and negative prompt boxes are filled with RC content the moment the page opens, and update live whenever the dropdown or base prompt changes.

Rectangle tool — Gradio's ImageEditor doesn't have a native rect tool, so both panels now have a collapsible "Or enter exact pixel coordinates" accordion with x1/y1/x2/y2 number inputs. These override the painted region when any value is non-zero. The brush size was increased to 20px so freehand painting covers an area quickly.

Object label removed from the smoke test as requested.


Pushed — commit a6a2515. Now "Choose style…" maps to "" (empty), so both prompt boxes stay blank until the user picks an actual domain.